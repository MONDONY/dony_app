...
- État "complet" (`availableKg <= 0`) : prix et libellé "Complet" (au lieu de "X kg dispo") passent en `cs.onSurfaceVariant`/`cs.error`.
- **Mise à jour post-livraison (retour utilisateur en test device) :** le bouton "Réserver" est désormais désactivé (`onPressed: null`) quand `availableKg <= 0` — `TravelerAnnouncementCard` ne permettait auparavant aucune désactivation (`onReserve` toujours appelable), ce qui laissait un CTA actif sur un trajet complet. Décision revue : un trajet complet ne doit pas pouvoir être "réservé". Voir commit `39b2ad8c`.
