...
class RequestTicketCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final r = request;
    final hasFooterDetails = r.acceptedPaymentMethods.isNotEmpty ||
        r.pickupNeighborhood != null || r.deliveryNeighborhood != null;
    ...
    return Container(
      ...
      child: Opacity(
        ...
        child: Column(
          ...
          children: [
            Padding(
              padding: const EdgeInsets.fromLTRB(DonySpacing.base, DonySpacing.base, DonySpacing.base, 0),
              child: Row(
                children: [
                  statusPill,
                  const Spacer(),
                  Flexible(
                    child: Text(metaLabel, maxLines: 1, overflow: TextOverflow.ellipsis,
            ...
            if (hasFooterDetails) _FooterDetails(key: const Key('request-ticket-footer-details'), request: r),
            ?footer,
          ],
        ),
      ),
...
class _Route extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final dateLabel = request.dateToleranceDays > 0 ? '$date ± ${request.dateToleranceDays} j' : date;
    ...
    return Semantics(
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            Expanded(child: end(request.departureCity, CrossAxisAlignment.start)),
            Column(
              children: [
        ...
        children: [
            ...
            child: LayoutBuilder(
              builder: (_, constraints) {
                ...
                return Row(
                  ...
                  children: List.generate(dashes, (_) => Container(width: 5, height: 1.5, color: cs.outline)),
                );
              },
            ),
          ),
          for (final left in [true, false])
            Positioned(
              ...
              child: Container(
                width: 20, height: 20,
                decoration: BoxDecoration(color: ground, shape: BoxShape.circle, border: Border.all(color: cs.outline)),
              ),
            ),
        ],
      ),
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(facts, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700)),
                ...
                Wrap(
                  ...
                  spacing: 6,
                  children: [
                    Text(price == null ? 'Prix à définir' : formatPriceIn(price, r.currency),
                        style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w800,
                            fontFeatures: [FontFeature.tabularFigures()])),
                    Text(r.negotiable ? 'négociable' : 'prix ferme',
                        style: TextStyle(fontSize: 12, color: cs.onSurfaceVariant)),
                  ],
                ),
                if (r.convertedDisplayPrice != null && r.convertedCurrency != null)
                  Text('≈ ${formatPriceIn(r.convertedDisplayPrice!, r.convertedCurrency)}',
                      style: TextStyle(fontSize: 12, color: cs.onSurfaceVariant)),
              ],
            ),
          ),
...
class _FooterDetails extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      padding: const EdgeInsets.fromLTRB(DonySpacing.base, DonySpacing.md, DonySpacing.base, DonySpacing.base),
      ...
      child: Column(
        ...
        children: [
          ...
          if (r.acceptedPaymentMethods.isNotEmpty) PaymentMethodsChips(methods: r.acceptedPaymentMethods),
        ],
      ),
    );
  }
