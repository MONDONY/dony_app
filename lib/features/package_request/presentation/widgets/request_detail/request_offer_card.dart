...
({String label, OfferTagTone tone, String? cta}) offerTagFor(NegotiationThread t, {required bool firmPrice}) {
  final name = t.travelerName ?? 'Le voyageur';
  return switch (t.status) {
    NegotiationThreadStatus.awaitingTrip => (label: '$name ajoute son trajet', tone: OfferTagTone.info, cta: null),
    NegotiationThreadStatus.awaitingPayment ||
    NegotiationThreadStatus.awaitingDeposit => (label: 'Accord trouvé', tone: OfferTagTone.success, cta: null),
    NegotiationThreadStatus.awaitingCommission =>
      (label: 'Accord en espèces, commission en attente', tone: OfferTagTone.warning, cta: null),
    _ when firmPrice => (label: 'Disponible pour ton colis', tone: OfferTagTone.success, cta: 'Choisir'),
    _ when t.isMyTurn => (label: 'À toi de répondre', tone: OfferTagTone.info, cta: 'Répondre'),
...
class RequestOfferCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final price = PriceDisplay.money(t.grossPriceEur ?? PriceDisplay.grossFromNet(t.currentPriceEur), t.currency);
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(
              color: highlighted ? cs.primary.withValues(alpha: 0.30) : cs.outline,
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  DonyAvatar(name: name, imageUrl: t.travelerPhotoUrl, verified: (t.travelerTripsCount ?? 0) > 0),
                      ...
                      children: [
                        Row(children: [
                          ...
                          if (t.travelerRating != null) ...[
                            ...
                            Text(NumberFormat('0.0', 'fr').format(t.travelerRating),
                                style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600)),
                          ],
                        ]),
                        Text('${DateFormat('d MMM', 'fr').format(t.travelerTravelDate)} · '
                  ...
                  Column(
                    ...
                    children: [
                      ...
                      Text('tu paies', style: TextStyle(fontSize: 12, color: cs.onSurfaceVariant)),
                    ],
                  ),
                ],
              ),
              ...
              Container(
                ...
                decoration: BoxDecoration(color: tagBg, borderRadius: BorderRadius.circular(DonyRadius.sm)),
                child: Row(
                  children: [
                    Expanded(child: Text(tag.label,
                        style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: tagFg))),
                    if (tag.cta != null)
                      Container(
                        ...
                        child: Text(tag.cta!, style: TextStyle(color: cs.onPrimary, fontSize: 13, fontWeight: FontWeight.w700)),
                      ),
                  ],
                ),
              ),
