...
String verdictFor(Map<String, dynamic> s) {
  ...
  final jankBuild =
      (s['missed_frame_build_budget_count'] as num? ?? 0) / frames;
  final jankRaster =
      (s['missed_frame_rasterizer_budget_count'] as num? ?? 0) / frames;
...
String renderReport(Map<String, Map<String, dynamic>> byScenario) {
  final b = StringBuffer('# Perf report (FPS / jank)\n\n')
    ..writeln(
      '| Scénario | avg build | worst build | %jank | avg raster | Verdict |',
    )
    ..writeln('|---|---|---|---|---|---|');
  byScenario.forEach((name, s) {
    final frames = (s['frame_count'] as num?)?.toInt() ?? 1;
    final jank =
        ((s['missed_frame_build_budget_count'] as num? ?? 0) / frames * 100)
            .toStringAsFixed(1);
    b.writeln(
      '| $name | ${s['average_frame_build_time_millis']} | '
...
void main(List<String> args) {
  ...
  stdout.writeln(
    'wrote reports/perf-report.md (${byScenario.length} scénarios)',
  );
}
