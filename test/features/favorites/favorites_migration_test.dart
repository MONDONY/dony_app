...
void main() {
  ...
  group('FavoritesMigration.run()', () {
    ...
    test(
      'swallows per-item repo.add error and still deletes the key',
      () async {
        when(() => box.get('saved_trips')).thenReturn([
          {'id': 'trip-ok'},
          {'id': 'trip-fail'},
    ...
    test(
      'is idempotent — second run is a no-op (key already deleted)',
      () async {
        // First call: key present, migration runs
        when(() => box.get('saved_trips')).thenReturn([
          <String, dynamic>{'id': 'trip-1'},
        ]);
