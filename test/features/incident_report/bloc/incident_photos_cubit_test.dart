...
class MockIncidentReportRepository extends Mock implements IncidentReportRepository {}
...
void main() {
  ...
  group('add', () {
    blocTest<IncidentPhotosCubit, List<IncidentPhotoUpload>>(
      ...
      build: () {
        when(() => repository.uploadPhoto('/tmp/a.png'))
      ...
      expect: () => [
        [
          isA<IncidentPhotoUpload>()
              .having((p) => p.status, 'status', IncidentPhotoUploadStatus.uploading),
        ],
        [
          isA<IncidentPhotoUpload>()
              .having((p) => p.status, 'status', IncidentPhotoUploadStatus.ready)
    ...
    blocTest<IncidentPhotosCubit, List<IncidentPhotoUpload>>(
      ...
      build: () {
        when(() => repository.uploadPhoto(any())).thenThrow(Exception('réseau'));
        return build();
      },
      ...
      expect: () => [
        ...
        [
          isA<IncidentPhotoUpload>()
              .having((p) => p.status, 'status', IncidentPhotoUploadStatus.failed),
        ],
      ],
    );

    test('refuse la 5e photo', () async {
      when(() => repository.uploadPhoto(any()))
  ...
  group('remove / getters', () {
    test('remove retire la photo ; readyKeys et hasUploading cohérents', () async {
      when(() => repository.uploadPhoto('/tmp/ok.png'))
          .thenAnswer((_) async => 'reports/u/ok.png');
      when(() => repository.uploadPhoto('/tmp/ko.png')).thenThrow(Exception());
      final cubit = build();
