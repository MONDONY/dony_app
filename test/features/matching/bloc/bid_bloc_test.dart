...
void main() {
  ...
  // ─── BidGridItemIncrementRequested / BidGridItemDecrementRequested ──────────

  group('BidGridItemIncrementRequested / BidGridItemDecrementRequested', () {
    blocTest<BidBloc, BidState>(
      'incrément → quantité 1, incrément → 2, décrément → 1',
      build: () => buildBloc(),
      act: (b) {
        b.add(const BidGridItemIncrementRequested('item-1'));
        b.add(const BidGridItemIncrementRequested('item-1'));
        b.add(const BidGridItemDecrementRequested('item-1'));
      },
      expect: () => [
        isA<BidGridQuantitiesUpdated>()
            .having((s) => s.gridQuantities['item-1'], 'qty après +1', 1),
        isA<BidGridQuantitiesUpdated>()
            .having((s) => s.gridQuantities['item-1'], 'qty après +2', 2),
        isA<BidGridQuantitiesUpdated>()
            .having((s) => s.gridQuantities['item-1'], 'qty après -1', 1),
      ],
    );
    ...
    blocTest<BidBloc, BidState>(
      'décrément à 1 retire l\'entrée de gridQuantities',
      build: () => buildBloc(),
      act: (b) {
        b.add(const BidGridItemIncrementRequested('item-2'));
        b.add(const BidGridItemDecrementRequested('item-2'));
      },
      expect: () => [
        isA<BidGridQuantitiesUpdated>()
            .having((s) => s.gridQuantities['item-2'], 'qty après +1', 1),
        isA<BidGridQuantitiesUpdated>().having(
            (s) => s.gridQuantities.containsKey('item-2'), 'absent', false),
      ],
    );

    blocTest<BidBloc, BidState>(
      'décrément sans incrément préalable n\'émet pas d\'état négatif',
      build: () => buildBloc(),
      act: (b) => b.add(const BidGridItemDecrementRequested('item-3')),
      expect: () => [
        isA<BidGridQuantitiesUpdated>().having(
            (s) => s.gridQuantities.containsKey('item-3'), 'absent', false),
      ],
    );

    blocTest<BidBloc, BidState>(
      'plusieurs articles indépendants gérés séparément',
      build: () => buildBloc(),
      act: (b) {
        b.add(const BidGridItemIncrementRequested('item-1'));
        b.add(const BidGridItemIncrementRequested('item-2'));
      ...
      expect: () => [
        isA<BidGridQuantitiesUpdated>()
            .having((s) => s.gridQuantities['item-1'], 'item-1 = 1', 1),
        isA<BidGridQuantitiesUpdated>()
            .having((s) => s.gridQuantities['item-2'], 'item-2 = 1', 1),
        isA<BidGridQuantitiesUpdated>()
            .having((s) => s.gridQuantities['item-1'], 'item-1 = 2', 2),
      ],
    );

    test('gridQuantities getter expose une map non modifiable', () {
      final bloc = buildBloc();
      bloc.add(const BidGridItemIncrementRequested('item-x'));
      expect(() => bloc.gridQuantities['item-x'] = 999, throwsA(anything));
    });
  });
