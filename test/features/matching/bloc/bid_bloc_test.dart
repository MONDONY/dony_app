...
BidModel buildBid({String id = 'bid-001', String status = 'PENDING'}) =>
    BidModel(
      id: id,
...
void main() {
  ...
  group('BidCheckoutRequested', () {
    ...
    blocTest<BidBloc, BidState>(
      ...
      expect: () => [
        isA<BidLoading>(),
        predicate<BidState>(
          (s) => s is BidError && s.error.message == 'Valeur maximum : 500 €',
        ),
      ],
    );
  ...
  group('BidListRequested', () {
    blocTest<BidBloc, BidState>(
      ...
      build: () {
        when(
          () => mockRepo.getBidsForAnnouncement('ann-001'),
    ...
    blocTest<BidBloc, BidState>(
      ...
      build: () {
        when(() => mockRepo.getBidsForAnnouncement(any())).thenThrow(
          DioException(
            requestOptions: RequestOptions(path: '/bids'),
      ...
      expect: () => [
        isA<BidLoading>(),
        predicate<BidState>(
          (s) => s is BidError && s.error.message == 'Accès refusé',
        ),
      ],
    );
    ...
    blocTest<BidBloc, BidState>(
      ...
      build: () {
        when(
          () => mockRepo.getBidsForAnnouncement(any()),
  ...
  group('BidMyListRequested', () {
    ...
    blocTest<BidBloc, BidState>(
      ...
      build: () {
        when(() => mockRepo.getMyBids()).thenThrow(
          DioException(
      ...
      expect: () => [
        isA<BidLoading>(),
        predicate<BidState>(
          (s) => s is BidError && s.error.message == 'Serveur indisponible',
        ),
      ],
    );
  ...
  group('BidDetailRequested', () {
    blocTest<BidBloc, BidState>(
      ...
      expect: () => [
        predicate<BidState>(
          (s) => s is BidDetailLoaded && s.bid.id == 'bid-001',
        ),
      ],
    );
    ...
    blocTest<BidBloc, BidState>(
      ...
      build: () {
        when(() => mockRepo.getBidById(any())).thenThrow(
          DioException(requestOptions: RequestOptions(path: '/bids/bid-999')),
        );
        return buildBloc();
      },
  ...
  group('BidAcceptRequested', () {
    ...
    blocTest<BidBloc, BidState>(
      ...
      build: () {
        when(() => mockRepo.acceptBid(any())).thenThrow(
          DioException(
  ...
  group('BidRejectRequested', () {
    blocTest<BidBloc, BidState>(
      ...
      build: () {
        when(
          () => mockRepo.rejectBid('bid-001', reason: any(named: 'reason')),
        ).thenAnswer((_) async => buildBid(status: 'REJECTED'));
        return buildBloc();
      },
      act: (bloc) =>
          bloc.add(BidRejectRequested('bid-001', reason: 'Trop lourd')),
      expect: () => [
        isA<BidLoading>(),
        predicate<BidState>(
          (s) => s is BidRejected && s.bid.status == 'REJECTED',
        ),
      ],
    );
    ...
    blocTest<BidBloc, BidState>(
      ...
      build: () {
        when(
          () => mockRepo.rejectBid(any(), reason: any(named: 'reason')),
        ).thenThrow(
          DioException(
      ...
      expect: () => [
        isA<BidLoading>(),
        predicate<BidState>(
          (s) => s is BidError && s.error.message == 'Déjà rejeté',
        ),
      ],
    );
  ...
  group('BidCancelRequested', () {
    blocTest<BidBloc, BidState>(
      ...
      expect: () => [
        isA<BidLoading>(),
        predicate<BidState>(
          (s) => s is BidCancelled && s.bid.status == 'CANCELLED',
        ),
      ],
    );
    ...
    blocTest<BidBloc, BidState>(
      ...
      build: () {
        when(
          () => mockRepo.cancelBid(any(), reason: any(named: 'reason')),
        ).thenThrow(
          DioException(
  ...
  group('BidHideRequested', () {
    ...
    blocTest<BidBloc, BidState>(
      'DioException → [Loading, BidError avec detail]',
      build: () {
        when(() => mockRepo.hideBid(any())).thenThrow(
          DioException(
  ...
  group('BidTravelerDismissRequested', () {
    blocTest<BidBloc, BidState>(
      ...
      build: () {
        when(
          () => mockRepo.dismissBidAsTraveler('bid-001'),
        ).thenAnswer((_) async {});
        return buildBloc();
      },
    ...
    blocTest<BidBloc, BidState>(
      'DioException → [Loading, BidError avec detail]',
      build: () {
        when(() => mockRepo.dismissBidAsTraveler(any())).thenThrow(
          DioException(
      ...
      expect: () => [
        isA<BidLoading>(),
        predicate<BidState>(
          (s) => s is BidError && s.error.message == 'Action non autorisée',
        ),
      ],
    );
    ...
    blocTest<BidBloc, BidState>(
      ...
      build: () {
        when(
          () => mockRepo.dismissBidAsTraveler(any()),
        ).thenThrow(Exception('timeout'));
        return buildBloc();
      },
      act: (bloc) => bloc.add(BidTravelerDismissRequested('bid-001')),
  ...
  group('BidConfirmPresenceRequested', () {
    blocTest<BidBloc, BidState>(
      ...
      build: () {
        when(
          () => mockRepo.confirmPresence('bid-001'),
        ).thenAnswer((_) async => buildBid(status: 'PRESENT'));
        return buildBloc();
      },
      act: (bloc) => bloc.add(BidConfirmPresenceRequested('bid-001')),
      expect: () => [isA<BidLoading>(), isA<BidPresenceConfirmed>()],
    );

    blocTest<BidBloc, BidState>(
      'DioException → [Loading, BidError avec detail]',
      build: () {
        when(() => mockRepo.confirmPresence(any())).thenThrow(
          DioException(
  ...
  group('BidConfirmPaymentRequested', () {
    ...
    blocTest<BidBloc, BidState>(
      ...
      build: () {
        when(() => mockRepo.confirmPayment(any())).thenThrow(
          DioException(
            requestOptions: RequestOptions(
              path: '/bids/bid-001/confirm-payment',
            ),
    ...
    blocTest<BidBloc, BidState>(
      ...
      build: () {
        when(() => mockRepo.confirmPayment(any())).thenThrow(Exception('boom'));
        return buildBloc();
      },
      act: (bloc) => bloc.add(BidConfirmPaymentRequested('bid-001')),
      expect: () => [isA<BidError>()],
  ...
  group('BidMyListAutoRefreshRequested', () {
    ...
    blocTest<BidBloc, BidState>(
      ...
      build: () {
        when(() => mockRepo.getMyBids()).thenThrow(
          DioException(requestOptions: RequestOptions(path: '/bids/my')),
        );
        return buildBloc();
      },
      seed: () => BidListLoaded([
      ...
      expect: () => [
        ...
        predicate<BidState>(
          (s) =>
              s is BidListLoaded && !s.isRefreshing && s.bids.first.id == 'old',
        ),
      ],
    );
  ...
  group('BidListRequested — 404', () {
    blocTest<BidBloc, BidState>(
      ...
      expect: () => [isA<BidLoading>(), isA<BidNotFound>()],
    );
  });
  ...
  group('BidModel.resolvedSenderName', () {
    ...
    test(
      'sans senderName → « Expéditeur » (le numéro ne sert plus de repli)',
      () {
        final bid = BidModel(
          id: 'b1',
          announcementId: 'a1',
  ...
  group('BidQuoteRequested', () {
    ...
    blocTest<BidBloc, BidState>(
      ...
      act: (bloc) =>
          bloc.add(BidQuoteRequested(announcementId: 'ann-001', weightKg: 5.0)),
      expect: () => [isA<BidQuoteLoading>(), isA<BidQuoteLoaded>()],
    );
