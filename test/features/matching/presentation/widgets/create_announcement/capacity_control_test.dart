...
void main() {
  group('CapacityControl', () {
    ...
    testWidgets('état initial : chip suitcase23kg sélectionné — pas de Slider',
        (tester) async {
      await tester.pumpWidget(_host());
      expect(find.text('1 valise 23 kg'), findsOneWidget);
    ...
    testWidgets('Personnalisé → champ de saisie libre visible (pas de Slider)',
        (tester) async {
      await tester.pumpWidget(_host());
    ...
    testWidgets(
        'compteur + (suitcase23kg) → availableKg passe à 46 (2×23)',
        (tester) async {
      final bloc = AnnouncementFormBloc();
      addTearDown(bloc.close);
    ...
    testWidgets(
        'compteur − depuis 2 → availableKg revient à 23 (1×23)',
        (tester) async {
      final bloc = AnnouncementFormBloc();
      addTearDown(bloc.close);
    ...
    testWidgets(
        'compteur − désactivé/inopérant à quantité 1 (suitcase23kg)',
        (tester) async {
      final bloc = AnnouncementFormBloc();
      addTearDown(bloc.close);
    ...
    testWidgets(
        'compteur affiche "1 valise" (singulier) pour quantité 1',
        (tester) async {
      final bloc = AnnouncementFormBloc();
      addTearDown(bloc.close);
