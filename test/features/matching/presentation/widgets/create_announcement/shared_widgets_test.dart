...
void main() {
  ...
  group('CaFieldCard', () {
    testWidgets('se construit et affiche son enfant', (tester) async {
      await tester.pumpWidget(_wrap(const CaFieldCard(child: Text('champ'))));
      expect(find.text('champ'), findsOneWidget);
    });

    testWidgets('utilise un ClipRRect et un Container', (tester) async {
      await tester.pumpWidget(_wrap(const CaFieldCard(child: SizedBox())));
  ...
  group('CaFieldIcon', () {
    testWidgets('se construit avec une icône', (tester) async {
      await tester.pumpWidget(_wrap(const CaFieldIcon(icon: Icons.search)));
      expect(find.byType(Icon), findsOneWidget);
    });

    testWidgets('rend un Icon avec la taille 20', (tester) async {
      await tester.pumpWidget(_wrap(const CaFieldIcon(icon: Icons.flight)));
  ...
  group('CaInlineAddRow', () {
    testWidgets('se construit avec un hint', (tester) async {
      ...
      await tester.pumpWidget(
        _wrap(CaInlineAddRow(controller: ctrl, hint: 'Ajouter…', onAdd: () {})),
      );
    ...
    testWidgets(
      'accentColor est utilisée pour colorier le bouton (défaut primary)',
      (tester) async {
        final ctrl = TextEditingController();
        await tester.pumpWidget(
          _wrap(
  ...
  group('CaRemovableChip', () {
    ...
    testWidgets('onRemove est appelé quand on tape sur la croix', (
      tester,
    ) async {
      var removed = false;
      await tester.pumpWidget(
  ...
  // ── CaTimeRow ────────────────────────────────────────────────────────────────
  group('CaTimeRow', () {
    testWidgets('affiche le placeholder départ quand time est null', (
      tester,
    ) async {
      await tester.pumpWidget(
        _wrap(CaTimeRow(isDeparture: true, time: null, onTap: () {})),
      );
      expect(find.text('Heure de départ (optionnel)'), findsOneWidget);
    });

    testWidgets(
      'affiche le placeholder arrivée quand time est null et isDeparture=false',
      (tester) async {
        await tester.pumpWidget(
          _wrap(CaTimeRow(isDeparture: false, time: null, onTap: () {})),
        );
        expect(find.text("Heure d'arrivée (optionnel)"), findsOneWidget);
      },
    );

    testWidgets('affiche l\'heure formatée quand time est fourni', (
      tester,
    ) async {
      await tester.pumpWidget(
        _wrap(
    ...
    testWidgets('onTap est appelé au tap', (tester) async {
      ...
      await tester.pumpWidget(
        _wrap(
          CaTimeRow(isDeparture: true, time: null, onTap: () => tapped = true),
        ),
      );
    ...
    testWidgets(
      'onClear est appelé quand time est fourni et onClear est tapé',
      (tester) async {
        var cleared = false;
        await tester.pumpWidget(
          _wrap(
    ...
    testWidgets('affiche une flèche (chevron) quand time est null', (
      tester,
    ) async {
      await tester.pumpWidget(
        _wrap(CaTimeRow(isDeparture: false, time: null, onTap: () {})),
  ...
  group('CaDateRow', () {
    testWidgets('affiche le placeholder quand date est null', (tester) async {
      await tester.pumpWidget(_wrap(CaDateRow(date: null, onTap: () {})));
      expect(find.text('Date de départ'), findsOneWidget);
    });

    testWidgets('affiche la date formatée quand date est fournie', (
      tester,
    ) async {
      await tester.pumpWidget(
        _wrap(CaDateRow(date: DateTime(2026, 8, 15), onTap: () {})),
      );
      ...
      expect(
        hasDateText,
        isTrue,
        reason: 'La date 2026-08-15 doit être affichée',
      );
    });
    ...
    testWidgets('onTap est appelé au tap', (tester) async {
      var tapped = false;
      await tester.pumpWidget(
        _wrap(CaDateRow(date: null, onTap: () => tapped = true)),
      );
      await tester.tap(find.byType(InkWell));
      await tester.pump();
      expect(tapped, isTrue);
