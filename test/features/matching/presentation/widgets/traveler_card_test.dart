...
void main() {
  ...
  group('TravelerCard', () {
    ...
    testWidgets('avatar shows gold verified badge when pro + kycVerified', (
      ...
      await tester.pumpAndSettle();
      final goldIcons = tester
          .widgetList<DonyIcon>(
            find.byWidgetPredicate(
              (w) => w is DonyIcon && w.name == 'badge-check',
            ),
          )
          .where((icon) => icon.color == const Color(0xFFF0B829));
      expect(goldIcons, isNotEmpty);
    });

    testWidgets(
      'avatar shows blue verified badge when kycVerified but NOT pro',
      (tester) async {
        await tester.pumpWidget(
          _wrap(
            TravelerCard(
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        final chevrons = tester.widgetList<DonyIcon>(
          find.byWidgetPredicate(
  ...
  group('TravelerCard – trajet (route header)', () {
    ...
    testWidgets('appui réduit légèrement la carte (scale < 1)', (tester) async {
      ...
      final gesture = await tester.startGesture(
        tester.getCenter(find.byType(TravelerCard)),
      );
  ...
  group('TravelerCard – distanceBadge', () {
    testWidgets('affiche le badge quand distanceBadge est fourni', (
      tester,
    ) async {
      await tester.pumpWidget(
        _wrap(
  ...
  group('TravelerCard – showFavorite', () {
    ...
    testWidgets('showFavorite=true avec cubit seeded favori : signet rempli', (
      tester,
    ) async {
      final repo = _MockFavoriteRepository();
  ...
  group('TravelerCard – badge urgent', () {
    testWidgets('affiche le badge Urgent quand annonce.isUrgent', (
      tester,
    ) async {
      await tester.pumpWidget(
        _wrap(
  ...
  group('TravelerCard — badges alignés liste/carousel', () {
    ...
    Future<void> expectBadgesSameRow(WidgetTester tester, double width) async {
      ...
      expect(
        ...
        pro,
        reason:
            'PRO et Urgent doivent être sur la même ligne à '
    ...
    testWidgets('une seule catégorie : pas de pastille « +N »', (tester) async {
      await tester.pumpWidget(
        _wrap(
          TravelerCard(
            announcement: _makeAnn(
              acceptedContentTypes: const ['Vêtements & tissus'],
            ),
            index: 0,
            isOwnAnnouncement: false,
            onTap: () {},
