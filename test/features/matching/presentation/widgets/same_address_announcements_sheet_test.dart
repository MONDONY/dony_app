...
Widget _wrap(Widget child, {FavoriteIdsCubit? favCubit}) {
  ...
  if (favCubit != null) {
    return BlocProvider<FavoriteIdsCubit>.value(value: favCubit, child: inner);
  }
  return inner;
}
...
void main() {
  ...
  testWidgets('tap on item invokes onTap with that announcement', (
    tester,
  ) async {
    String? tappedId;
    final list = [_ann('a1', 'Paris', 'Dakar'), _ann('a2', 'Paris', 'Dakar')];
      ...
      await tester.pumpWidget(
        _wrap(
          SameAddressAnnouncementsSheet(
            ...
            announcements: list,
            currentUserId:
                't1', // matches travelerId → isOwn=true → showFavorite:false
            onTap: (_) {},
          ),
          favCubit: cubit,
        ),
