...
void main() {
  ...
  testWidgets('StatusChipsRow : chip active mise en avant, tap change',
    ...
    await tester.pumpWidget(_wrap(StatusChipsRow<TripStatusFilter>(
      chips: const [
  ...
  testWidgets('StatusChipsRow avec equals (Set) compare correctement',
      (tester) async {
    Set<String>? selected;
    await tester.pumpWidget(_wrap(StatusChipsRow<Set<String>>(
      chips: const [
  ...
  testWidgets('ActivitySearchField émet onChanged', (tester) async {
    String? typed;
    await tester.pumpWidget(_wrap(ActivitySearchField(
      hint: 'Rechercher…',
      onChanged: (v) => typed = v,
    )));
