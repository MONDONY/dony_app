...
void main() {
  ...
  test(
    'cancellation_initiated fires with reason on CancellationTripRequested success',
    () async {
      when(
        () => repo.cancelTrip(
      ...
      verify(
        () => backend.capture(AnalyticsEvents.cancellationInitiated, {
          'reason': 'changed_mind',
        }),
      ).called(1);
    },
  ...
  test(
    'no_show_reported_by_sender fires on TravelerNoShowReportRequested success',
    () async {
      ...
      verify(
        () => backend.capture(AnalyticsEvents.noShowReportedBySender, any()),
      ).called(1);
    },
  );

  test(
    'no_show_reported_by_traveler fires on NoShowReportRequested success',
    () async {
      ...
      verify(
        () => backend.capture(AnalyticsEvents.noShowReportedByTraveler, any()),
      ).called(1);
    },
  );

  test(
    'delivery_no_show_reported_by_traveler fires on DeliveryNoShowReportRequested success',
    () async {
      ...
      verify(
        () => backend.capture(
          AnalyticsEvents.deliveryNoShowReportedByTraveler,
          any(),
        ),
      ).called(1);
    },
  ...
  test(
    'delivery_no_show_reported_by_sender fires on TravelerDeliveryNoShowReportRequested success',
    () async {
      when(
        () => repo.reportTravelerDeliveryNoShow(any()),
      ).thenAnswer((_) async {});
      ...
      verify(
        () => backend.capture(
          AnalyticsEvents.deliveryNoShowReportedBySender,
          any(),
        ),
      ).called(1);
    },
  ...
  test(
    'delivery_no_show_contested fires on DeliveryNoShowContestRequested success',
    () async {
      ...
      verify(
        () => backend.capture(AnalyticsEvents.deliveryNoShowContested, any()),
      ).called(1);
    },
  );
  ...
  test('no event when disabled', () async {
    ...
    bloc.add(
      CancellationTripRequested(announcementId: 'ann1', reason: 'changed_mind'),
    );
    await bloc.stream.firstWhere((s) => s is CancellationSuccess);
    await Future<void>.delayed(Duration.zero);
