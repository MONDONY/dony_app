...
void main() {
  ...
  testWidgets('déclenche le fetch avec le bon cancellationId au démarrage', (
    tester,
  ) async {
    ...
    verify(
      () => bloc.add(
        any(
          that: isA<RematchSuggestionsRequested>().having(
            (e) => e.cancellationId,
            'cancellationId',
            'canc-42',
          ),
  ...
  testWidgets(
    ...
    'self-fetching) et affiche le bouton de signalement de bug',
    (tester) async {
      when(() => bloc.state).thenReturn(CancellationInitial());
  ...
  testWidgets('rend une TravelerCard par suggestion avec la note voyageur', (
    ...
    expect(
      card.announcement.traveler?.avatarUrl,
      'https://s3.example/avatar-awa.jpg',
    );
  });

  testWidgets('sans note voyageur, la TravelerCard affiche le repli "-"', (
    tester,
  ) async {
    final noRating = [
      RematchSuggestionModel(
  ...
  testWidgets('affiche un état vide explicite quand aucune suggestion', (
    tester,
  ) async {
  ...
  testWidgets(
    'affiche une erreur avec retry sur CancellationError, et le retry redispatch',
    (tester) async {
      when(
        () => bloc.state,
    ...
    (tester) async {
      ...
      verify(
        () => announcementBloc.add(
          any(
            that: isA<AnnouncementDetailRequested>().having(
              (e) => e.id,
              'id',
              'ann-2',
            ),
