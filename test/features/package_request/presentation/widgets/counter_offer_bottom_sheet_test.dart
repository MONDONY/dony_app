...
void main() {
  ...
  setUpAll(() {
    registerFallbackValue(
      const NegotiationCounterRequested(threadId: 't-1', proposedPriceEur: 40),
    );
  });
  ...
  group('CounterOfferBottomSheet', () {
    ...
    testWidgets(
      'affiche le bouton "Envoyer ma contre-offre" désactivé par défaut',
      (tester) async {
        await openSheet(tester);
    ...
    testWidgets(
      'le bouton envoie l\'événement après saisie d\'un prix valide',
      (tester) async {
        await openSheet(tester);
        ...
        verify(
          () => bloc.add(any(that: isA<NegotiationCounterRequested>())),
        ).called(1);
      },
    );
