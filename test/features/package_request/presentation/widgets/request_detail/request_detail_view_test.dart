...
Future<void> _pump(
  WidgetTester tester,
  PackageRequestDetailLoaded state, {
  RequestDetailCallbacks? cb,
}) => tester.pumpWidget(
  MaterialApp(
    theme: AppTheme.light(),
    home: Scaffold(
      body: SingleChildScrollView(
        child: RequestDetailView(state: state, callbacks: cb ?? _noop),
      ),
    ),
  ),
);
...
void main() {
  ...
  testWidgets('1 brouillon : bandeau pas visible + aperçu repliée', (
    tester,
  ) async {
    await _pump(
      tester,
      PackageRequestDetailLoaded(
        request: _req(PackageRequestStatus.draft),
        threads: const [],
  ...
  testWidgets(
    '2 ter publiée : recherche voyageurs en échec (compatibleTrips null) → état visible',
    (tester) async {
      await _pump(
        tester,
        PackageRequestDetailLoaded(
      ...
      expect(
        find.text('Impossible de charger les voyageurs pour le moment'),
        findsOneWidget,
      );
    },
  );
  ...
  testWidgets('4 offres reçues : à répondre en tête, voyageurs repliés', (
    tester,
  ) async {
    await _pump(
      tester,
      PackageRequestDetailLoaded(
        request: _req(PackageRequestStatus.negotiating),
        threads: [
          _t(NegotiationThreadStatus.open, id: 'b'),
    ...
    final cards = tester
        .widgetList<RequestOfferCard>(find.byType(RequestOfferCard))
        .toList();
    expect(cards.first.thread.isMyTurn, isTrue);
    expect(find.byType(RequestTravelersFold), findsOneWidget);
  });

  testWidgets('5 prix ferme : Choisir + rappel + pastille + voyageurs masqués', (
    tester,
  ) async {
    await _pump(
      tester,
      PackageRequestDetailLoaded(
        request: _req(PackageRequestStatus.open, negotiable: false),
    ...
    expect(
      find.text('Les autres candidats seront déclinés automatiquement.'),
      findsOneWidget,
    );
  ...
  testWidgets('6 commission espèces : bandeau ambre, autres offres visibles', (
    tester,
  ) async {
    await _pump(
      tester,
  ...
  testWidgets(
    '7 à finaliser : une seule offre + pastille + voyageurs masqués',
    (tester) async {
      await _pump(
        tester,
        PackageRequestDetailLoaded(
  ...
  testWidgets('8 acceptée : talon voyageur + frise', (tester) async {
    await _pump(
      tester,
      PackageRequestDetailLoaded(
        request: _req(PackageRequestStatus.accepted),
    ...
    expect(
      find.byKey(const Key('request-ticket-traveler-stub')),
      findsOneWidget,
    );
    expect(
      tester
          .widget<RequestProgressTimeline>(find.byType(RequestProgressTimeline))
          .currentStep,
      2,
    );
  ...
  testWidgets('8 ter acceptée : bid annulé → bandeau au lieu de la frise', (
    tester,
  ) async {
    await _pump(
      tester,
  ...
  testWidgets('8 bis acceptée en espèces : talon « à régler en main propre »', (
    tester,
  ) async {
    await _pump(
      tester,
  ...
  testWidgets('9 bis livrée en espèces : talon « réglé en main propre »', (
    tester,
  ) async {
    await _pump(
      tester,
  ...
  testWidgets('10 expirée : bandeau', (tester) async {
    await _pump(
      tester,
      PackageRequestDetailLoaded(
        request: _req(PackageRequestStatus.expired),
        threads: const [],
      ),
    );
  ...
  testWidgets('tap sur une offre ouvre le fil', (tester) async {
    ...
    await _pump(
      ...
      cb: RequestDetailCallbacks.noop().copyWith(
