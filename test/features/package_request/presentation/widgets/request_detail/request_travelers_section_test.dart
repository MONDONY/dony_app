...
AnnouncementModel _trip(String id) => AnnouncementModel(
  id: id, travelerId: 'tr-$id', departureCity: 'Divo', arrivalCity: 'Annemasse',
...
Widget _wrap(Widget w) => MaterialApp(theme: AppTheme.light(), home: Scaffold(body: SingleChildScrollView(child: w)));

void main() {
  setUpAll(() => initializeDateFormatting('fr'));

  testWidgets('liste : titre avec compteur, une carte par trajet', (tester) async {
    final invited = <String>[];
    await tester.pumpWidget(_wrap(RequestTravelersList(
      trips: [_trip('a'), _trip('b')], requestWeightKg: 2,
      inviteStateFor: (_) => TravelerInviteState.idle, onInvite: invited.add, onOpenTrip: (_) {})));
  ...
  testWidgets('état vide : alerte et dates', (tester) async {
    var alert = 0, widen = 0;
    await tester.pumpWidget(_wrap(RequestNoTravelersEmpty(
      corridor: 'Divo → Annemasse', onCreateAlert: () => alert++, onWidenDates: () => widen++)));
    expect(find.text('Aucun voyageur sur Divo → Annemasse pour l\'instant'), findsOneWidget);
