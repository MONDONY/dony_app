...
void main() {
  test('libellés', () {
    ...
    expect(requestPillFor(RequestScreenCase.noTravelers).tone, RequestPillTone.live);
    expect(requestPillFor(RequestScreenCase.offersReceived, count: 1).label, '1 offre');
    expect(requestPillFor(RequestScreenCase.offersReceived, count: 2).label, '2 offres');
    expect(requestPillFor(RequestScreenCase.firmCandidates, count: 2).label, '2 candidats');
    expect(requestPillFor(RequestScreenCase.cashCommissionPending).tone, RequestPillTone.warning);
    ...
    expect(requestPillFor(RequestScreenCase.cancelled).tone, RequestPillTone.danger);
  });
}
