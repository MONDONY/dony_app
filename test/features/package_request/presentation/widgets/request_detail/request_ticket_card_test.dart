...
PackageRequest _req({List<String> photos = const [], String? description = '3 flacons de parfum emballés',
    Set<PaymentMethod> methods = const {PaymentMethod.stripe, PaymentMethod.cash}, bool negotiable = true}) => PackageRequest(
  ...
  categories: const ['Cosmétiques & parfums'], description: description,
  targetPriceEur: 29, status: PackageRequestStatus.open,
  createdAt: DateTime.utc(2026, 9, 17, 6, 25), negotiable: negotiable,
  pickupNeighborhood: 'Commerce', deliveryNeighborhood: 'Centre-ville',
  photoUrls: photos, photoKeys: [for (var i = 0; i < photos.length; i++) 'k$i'],
  acceptedPaymentMethods: methods,
);

Widget _wrap(Widget child) => MaterialApp(theme: AppTheme.light(), home: Scaffold(body: SingleChildScrollView(child: child)));

void main() {
  setUpAll(() => initializeDateFormatting('fr'));

  testWidgets('route, faits, quartiers et paiements', (tester) async {
    await tester.pumpWidget(_wrap(RequestTicketCard(
      request: _req(), statusPill: const Text('En ligne'), metaLabel: '14 vues · publiée il y a 2 h')));
    ...
    expect(find.bySemanticsLabel(RegExp('Divo vers Annemasse')), findsOneWidget);
    semantics.dispose();
  });

  testWidgets('sans photo : vignette colis ; prix ferme', (tester) async {
    await tester.pumpWidget(_wrap(RequestTicketCard(
      request: _req(negotiable: false, description: null), statusPill: const SizedBox(), metaLabel: '')));
    expect(find.byKey(const Key('request-ticket-photo-placeholder')), findsOneWidget);
    expect(find.text('prix ferme'), findsOneWidget);
  });

  testWidgets('plusieurs photos : compteur +N', (tester) async {
    await tester.pumpWidget(_wrap(RequestTicketCard(
      request: _req(photos: ['https://x/1.jpg', 'https://x/2.jpg', 'https://x/3.jpg']),
      statusPill: const SizedBox(), metaLabel: '')));
    expect(find.text('+2'), findsOneWidget);
  });

  testWidgets('sans moyen de paiement ni quartier : pied masqué ; footer affiché', (tester) async {
    await tester.pumpWidget(_wrap(RequestTicketCard(
      ...
      statusPill: const SizedBox(), metaLabel: '', footer: const Text('talon'))));
    expect(find.byKey(const Key('request-ticket-footer-details')), findsNothing);
