...
void main() {
  group('PaymentRecapContent — STRIPE payment', () {
    testWidgets('shows traveler net amount, service fee, and total to pay', (
      tester,
    ) async {
    ...
    testWidgets('shows escrow trust banner for stripe', (tester) async {
      await tester.pumpWidget(
        _wrapContent(net: 35.0, gross: 39.20, fee: 4.20, isCash: false),
      );

      // Trust banner contains the escrow message
      expect(find.textContaining('Sécurisé · bloqué jusqu'), findsOneWidget);
      expect(
  ...
  group('PaymentRecapContent — CASH payment', () {
    testWidgets('shows amount to hand over, fee note, and traveler net', (
      tester,
    ) async {
      await tester.pumpWidget(
    ...
    testWidgets('shows cash trust banner for cash', (tester) async {
      await tester.pumpWidget(
        _wrapContent(net: 35.0, gross: 39.20, fee: 4.20, isCash: true),
      );

      expect(find.textContaining('Paiement en main propre'), findsOneWidget);
      expect(
  ...
  group('PaymentRecapContent — CTA label derivation (via thread)', () {
    ...
    testWidgets('cash thread → CTA label should be "Confirmer l\'accord"', (
      tester,
    ) async {
  ...
  group('PaymentRecapContent — fee calculation from grossPriceEur=null', () {
    testWidgets('when grossPriceEur is null, fee is computed from net * 0.12', (
      tester,
    ) async {
