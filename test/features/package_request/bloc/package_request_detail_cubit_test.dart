...
void main() {
  ...
  PackageRequestDetailCubit build() => PackageRequestDetailCubit(
    requests,
    announcements,
    bids,
    analytics,
  ...
  setUp(() {
    ...
    when(
      () => requests.getById('pr-1'),
    ).thenAnswer((_) async => _req(PackageRequestStatus.open));
    when(
      () => requests.listThreadsForRequest('pr-1'),
    ).thenAnswer((_) async => const []);
    when(() => requests.getInsights('pr-1')).thenAnswer(
      (_) async => const PackageRequestInsights(
        viewCount: 14,
        invitedAnnouncementIds: {'a-1'},
      ),
    );
    stubSearch([
      _trip('a-1', 'trav-a'),
  ...
  blocTest<PackageRequestDetailCubit, PackageRequestDetailState>(
    ...
    expect: () => [
      ...
      isA<PackageRequestDetailLoaded>()
          ...
          .having((s) => s.compatibleTrips!.map((t) => t.id), 'trips', [
            'a-1',
            'a-2',
          ])
  ...
  blocTest<PackageRequestDetailCubit, PackageRequestDetailState>(
    ...
    skip: 1,
    expect: () => [
      isA<PackageRequestDetailLoaded>().having(
        (s) => s.compatibleTrips!.map((t) => t.id),
        'trips',
        ['a-2'],
      ),
    ],
  ...
  blocTest<PackageRequestDetailCubit, PackageRequestDetailState>(
    ...
    setUp: () {
      when(
        () => requests.getById('pr-1'),
      ).thenAnswer((_) async => _req(PackageRequestStatus.accepted));
      when(() => requests.listThreadsForRequest('pr-1')).thenAnswer(
        (_) async => [
  ...
  blocTest<PackageRequestDetailCubit, PackageRequestDetailState>(
    ...
    verify: (_) => verify(
      () => analytics.logEvent(AnalyticsEvents.packageRequestCancelled),
    ).called(1),
  );
  ...
  blocTest<PackageRequestDetailCubit, PackageRequestDetailState>(
    ...
    setUp: () {
      when(
        () => requests.getById('pr-1'),
      ).thenAnswer((_) async => _req(PackageRequestStatus.draft));
      when(
        () => requests.publish('pr-1'),
      ).thenAnswer((_) async => _req(PackageRequestStatus.open));
    },
    act: (c) async {
      await c.load();
  ...
  test(
    'invite et cancel entrelacés : l état relu après chaque await, pas l état capturé avant',
    () async {
      final inviteCompleter = Completer<InvitationOutcome>();
      final cancelCompleter = Completer<void>();
      when(
        () => requests.inviteTraveler('pr-1', 'a-2'),
      ).thenAnswer((_) => inviteCompleter.future);
      when(
        () => requests.cancel('pr-1'),
  ...
  blocTest<PackageRequestDetailCubit, PackageRequestDetailState>(
    ...
    setUp: () {
      ...
      when(
        () => requests.inviteTraveler('pr-1', 'a-2'),
      ).thenAnswer((_) async => InvitationOutcome.notFound);
    },
    act: (c) async {
      await c.load();
      await c.invite('a-2');
  ...
  blocTest<PackageRequestDetailCubit, PackageRequestDetailState>(
    ...
    setUp: () => when(
      () => requests.inviteTraveler('pr-1', 'a-2'),
    ).thenAnswer((_) async => InvitationOutcome.notInvitable),
    act: (c) async {
      await c.load();
      await c.invite('a-2');
    },
  ...
  blocTest<PackageRequestDetailCubit, PackageRequestDetailState>(
    ...
    setUp: () => when(
      () => requests.inviteTraveler('pr-1', 'a-2'),
    ).thenAnswer((_) async => InvitationOutcome.limitReached),
    act: (c) async {
      await c.load();
      await c.invite('a-2');
    },
  ...
  test('traces sans état', () async {
    ...
    verify(
      () => analytics.logEvent(AnalyticsEvents.packageRequestMenuOpened),
    ).called(1);
    verify(
      () => analytics.logEvent(
