...
void main() {
  ...
  test(
    'rating_submitted fires with score=5 and role_rated=traveler on RatingSubmitRequested',
    () async {
      when(
        () => repo.submitRating(
      ...
      verify(
        () => backend.capture(AnalyticsEvents.ratingSubmitted, {
          'score': 5,
          'role_rated': 'traveler',
        }),
      ).called(1);
  ...
  test(
    'rating_submitted fires with role_rated=sender on TravelerRatingSubmitRequested',
    () async {
      when(
        () => repo.submitTravelerRating(
          bidId: any(named: 'bidId'),
          stars: any(named: 'stars'),
          comment: any(named: 'comment'),
        ),
