...
void main() {
  ...
  test('getMySubscriptions parse la liste', () async {
    when(() => dio.get('/me/subscriptions')).thenAnswer((_) async => _ok([
          {
            'travelerId': 't1', 'travelerName': 'Ibrahima D', 'isProAccount': true,
            'averageRating': 4.8, 'ongoingTripsCount': 2, 'pushEnabled': false, 'hasNew': true,
            'lastAnnouncement': {
              'announcementId': 'a1', 'departureCity': 'Paris', 'arrivalCity': 'Dakar',
              'pricePerKg': 8.0, 'publishedAt': '2026-05-25T10:00:00'
            }
          }
        ], '/me/subscriptions'));
  ...
  test('getStatus parse subscribed/pushEnabled', () async {
    when(() => dio.get('/travelers/t1/subscription'))
        .thenAnswer((_) async => _ok({'subscribed': true, 'pushEnabled': false}, '/travelers/t1/subscription'));
  ...
  test('subscribe POST', () async {
    when(() => dio.post('/travelers/t1/subscribe')).thenAnswer((_) async => _ok(null, 'x'));
    await ds.subscribe('t1');
    verify(() => dio.post('/travelers/t1/subscribe')).called(1);
  });

  test('unsubscribe DELETE', () async {
    when(() => dio.delete('/travelers/t1/subscribe')).thenAnswer((_) async => _ok(null, 'x'));
    await ds.unsubscribe('t1');
    verify(() => dio.delete('/travelers/t1/subscribe')).called(1);
  });

  test('setPush PUT renvoie le statut', () async {
    when(() => dio.put('/travelers/t1/subscribe/push', data: any(named: 'data')))
        .thenAnswer((_) async => _ok({'subscribed': true, 'pushEnabled': true}, 'x'));
    final s = await ds.setPush('t1', true);
    expect(s.pushEnabled, true);
  });

  test('markSeen POST', () async {
    when(() => dio.post('/me/subscriptions/t1/mark-seen')).thenAnswer((_) async => _ok(null, 'x'));
    await ds.markSeen('t1');
    verify(() => dio.post('/me/subscriptions/t1/mark-seen')).called(1);
  });

  test('getTravelerAnnouncements parse', () async {
    when(() => dio.get('/travelers/t1/announcements')).thenAnswer((_) async => _ok([
          {'id': 'a1', 'departureCity': 'Paris', 'arrivalCity': 'Dakar',
           'departureDate': '2026-06-01', 'pricePerKg': 8.0, 'availableKg': 5.0, 'status': 'ACTIVE'}
