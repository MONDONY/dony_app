...
void main() {
  test('wire maps enum → backend string', () {
    expect(
      AlertDirection.travelerWantsPackages.wire,
  ...
  test('fromWire maps backend string → enum', () {
    expect(
      AlertDirection.fromWire('TRAVELER_WANTS_PACKAGES'),
      AlertDirection.travelerWantsPackages,
    );
    expect(
      AlertDirection.fromWire('SENDER_WANTS_TRIPS'),
  ...
  test('fromWire defaults to travelerWantsPackages for null/unknown', () {
    ...
    expect(
      AlertDirection.fromWire('GARBAGE'),
      AlertDirection.travelerWantsPackages,
    );
  });
}
