...
void main() {
  ...
  group('DonyEmptyState', () {
    ...
    testWidgets('empty type shows icon + title', (tester) async {
      await tester.pumpWidget(wrap(const DonyEmptyState(title: 'Aucun colis')));
    ...
    testWidgets('empty type shows CTA button when actionLabel provided', (
      tester,
    ) async {
      var pressed = false;
      await tester.pumpWidget(
    ...
    testWidgets('error type with CTA uses primary button variant', (
      tester,
    ) async {
      await tester.pumpWidget(
        wrap(
    ...
    testWidgets('custom icon overrides default', (tester) async {
      await tester.pumpWidget(
        wrap(
          const DonyEmptyState(title: 'Inbox vide', icon: Icons.mail_outline),
        ),
      );
      await tester.pumpAndSettle();
    ...
    testWidgets('custom padding is respected', (tester) async {
      await tester.pumpWidget(
        wrap(const DonyEmptyState(title: 'Vide', padding: EdgeInsets.all(8))),
      );
      await tester.pumpAndSettle();
    ...
    testWidgets('no CTA shown when actionLabel is null', (tester) async {
      await tester.pumpWidget(wrap(const DonyEmptyState(title: 'Vide')));
      await tester.pumpAndSettle();
  ...
  group('DonyEmptyState — param mascotte', () {
    testWidgets('mascotte remplace DonyIconContainer quand fournie', (
      tester,
    ) async {
      await tester.pumpWidget(
        wrap(
    ...
    testWidgets('mascotte absente → DonyIconContainer présent', (tester) async {
      await tester.pumpWidget(wrap(const DonyEmptyState(title: 'Aucun envoi')));
      await tester.pumpAndSettle();
