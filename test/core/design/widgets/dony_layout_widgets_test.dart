  ...
  final router = GoRouter(
    ...
    routes: [
      GoRoute(
        ...
        routes: [GoRoute(path: 'child', builder: (ctx, _) => builder(ctx))],
      ),
    ],
  );
...
void main() {
  ...
  group('DonyStepIndicator', () {
    ...
    group('dots variant', () {
      testWidgets('renders correct number of AnimatedContainers at step 0', (
        ...
        await tester.pumpWidget(
          _wrap(const DonyStepIndicator(total: 3, current: 0)),
        );
      ...
      testWidgets('renders with current step in the middle', (tester) async {
        await tester.pumpWidget(
          _wrap(const DonyStepIndicator(total: 4, current: 1)),
        );
        await tester.pump(const Duration(milliseconds: 600));
      ...
      testWidgets('renders with current step at the last position', (
        ...
        await tester.pumpWidget(
          _wrap(const DonyStepIndicator(total: 4, current: 3)),
        );
        await tester.pump(const Duration(milliseconds: 600));
        expect(find.byType(AnimatedContainer), findsNWidgets(4));
      });
      ...
      testWidgets('variant defaults to dots', (tester) async {
        await tester.pumpWidget(
          _wrap(const DonyStepIndicator(total: 2, current: 0)),
        );
        await tester.pump(const Duration(milliseconds: 600));
        final widget = tester.widget<DonyStepIndicator>(
          find.byType(DonyStepIndicator),
        );
    ...
    group('labeled variant', () {
      ...
      testWidgets('active step (current=0) shows no check icon', (
        tester,
      ) async {
        await tester.pumpWidget(
          _wrap(
            const DonyStepIndicator.labeled(steps: ['A', 'B', 'C'], current: 0),
          ),
        );
        await tester.pump(const Duration(milliseconds: 600));
      ...
      testWidgets('done steps show check icon (current=2)', (tester) async {
        await tester.pumpWidget(
          _wrap(
            const DonyStepIndicator.labeled(steps: ['A', 'B', 'C'], current: 2),
          ),
        );
        await tester.pump(const Duration(milliseconds: 600));
  ...
  group('DonyBottomSheet', () {
    ...
    testWidgets('shows subtitle when title and subtitle provided', (
      tester,
    ) async {
      await tester.pumpWidget(
        _sheetTrigger(
    ...
    testWidgets('shows close icon button when title is provided', (
      tester,
    ) async {
      await tester.pumpWidget(
    ...
    testWidgets(
      'M3 — stickyBottom est dans un SafeArea et un Padding horizontal',
      (tester) async {
        await tester.pumpWidget(
          MaterialApp(
            theme: AppTheme.light(),
  ...
  group('DonyPageScaffold', () {
    ...
    testWidgets('body is scrollable by default (SingleChildScrollView)', (
      ...
      await tester.pumpWidget(
        _wrapWithRouter(
          (_) =>
              const DonyPageScaffold(title: 'Scrollable', body: Text('Scroll')),
        ),
      );
      await tester.pumpAndSettle();
    ...
    testWidgets('shows back button by default', (tester) async {
      await tester.pumpWidget(
        _wrapWithRouter(
          (_) => const DonyPageScaffold(title: 'Back', body: Text('Has back')),
        ),
      );
      await tester.pumpAndSettle();
    ...
    testWidgets('onBack callback called when back button tapped', (
      tester,
    ) async {
    ...
    testWidgets('resizeToAvoidBottomInset is passed to Scaffold', (
      tester,
    ) async {
      await tester.pumpWidget(
        _wrapWithRouter(
  ...
  group('DonyLayout', () {
    ...
    testWidgets('isMedium returns true when 600 <= width < 840', (
      tester,
    ) async {
    ...
    testWidgets('maxContentWidth returns double.infinity on compact', (
      tester,
    ) async {
      tester.view.physicalSize = const Size(400, 800);
      tester.view.devicePixelRatio = 1.0;
    ...
    testWidgets('constrained returns child directly on compact', (
      tester,
    ) async {
      tester.view.physicalSize = const Size(400, 800);
      tester.view.devicePixelRatio = 1.0;
  ...
  group('DonyAppBar', () {
    ...
    testWidgets('calls onBack callback when back button tapped', (
      tester,
    ) async {
      var pressed = false;
      await tester.pumpWidget(
    ...
    testWidgets(
      'uses context.pop() when onBack is null and back button tapped',
      (tester) async {
        await tester.pumpWidget(
          _wrapWithRouter(
            (_) => const Scaffold(
    ...
    testWidgets(
      'preferredSize height adds bottom height when bottom provided',
      (tester) async {
  ...
  group('DonyStatusColors extension', () {
    ...
    testWidgets('errorLight returns light variant in light mode', (
      tester,
    ) async {
